...
export function TopSelling() {
  ...
  useEffect(()=>{
    const fetchData = async ()=>{
      try{
        setLoading(true)
        const res = await fetch("https://6828fc576075e87073a57861.mockapi.io/exam-products")
        if(!res.ok){
  ...
  return (
    <section className="py-16 ">
        ...
        <h2 className="text-4xl md:text-5xl font-bold text-center mb-12">TOP SELLING</h2>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {arrivals?.map((product) => (
          Number(product.id) > 4 && Number(product.id) <= 8 ?<Link href={`/products/${product.id}`} key={product.id} className="no-underline">
              ...
              <CardContent className="p-4">
                ...
                <div className="flex items-center mb-2">
                  {[...Array(5)].map((_, i) => {
                    ...
                    } else if (i === Math.floor(product.rate) && product.rate % 1 !== 0) {
                      return <StarHalf key={i} className="w-4 h-4 fill-yellow-400 text-yellow-400" />
                    } else {
                      return <Star key={i} className="w-4 h-4 text-gray-300" />
                    }
                  })}
                  <span className="text-sm text-gray-500 ml-1">{product.rate}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="font-bold text-xl">${product.currentPrice}</span>
                  {product.price && (
                    <>
